...
class Objects
{
	...
	public function allWhere($table)
	{
		$stmt = $this->pdo->prepare("SELECT * FROM " . $table . " WHERE quantity > 0");
		$stmt->execute();
		return $stmt->fetchAll(PDO::FETCH_OBJ);
	}
	...
	public function getInvoiceDetails($value)
	{
        ...
        SELECT 
            ...
            0 AS old_price,
			 p.id AS pid,
            p.hnscode AS hns_code,
            p.product_gst AS gst_percent,
            p.sell_price AS mrp,
            idet.quantity AS order_qty,
            idet.discount AS discount,
            idet.price AS total_price,
            idet.gst_price AS gst_price,
			idet.product_name AS product_name
        FROM invoice_details AS idet
        JOIN products AS p 
            ON idet.pid = p.id
        JOIN invoice AS inv
            ON idet.invoice_no = inv.id
        WHERE idet.invoice_no = :value
        ORDER BY idet.id DESC
    ");
		$stmt->bindParam(":value", $value);
		$stmt->execute();
		return $stmt->fetchAll(PDO::FETCH_OBJ);
	...
	public function storeCustomerOrderInvoice($invoice_number, $customer_id, $orderdate, $find_customer_name, $po_no, $po_date, $challan_no, $challan_date, $total_quantity, $orderQuantity, $price, $totalPrice, $pro_name, $pid, $gstPrice, $gst, $hnscode, $subtotal, $gsttotal, $billamount, $payMethode, $discount)
	{
		$pre_stmt = $this->pdo->prepare("INSERT INTO `invoice`(`invoice_number`,`customer_id`,`customer_name`, `order_date`, `po_no` ,`po_date` ,`challan_no`, `challan_date`, `sub_total`, `total_gst`,`bill_amount`, `payment_type`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)");

		$pre_stmt->bindParam(1, $invoice_number, PDO::PARAM_STR);
		$pre_stmt->bindParam(2, $customer_id, PDO::PARAM_INT);
		$pre_stmt->bindParam(3, $find_customer_name, PDO::PARAM_STR);
		$pre_stmt->bindParam(4, $orderdate, PDO::PARAM_STR);
		$pre_stmt->bindParam(5, $po_no, PDO::PARAM_INT);
		$pre_stmt->bindParam(6, $po_date, PDO::PARAM_STR);
		$pre_stmt->bindParam(7, $challan_no, PDO::PARAM_INT);
		$pre_stmt->bindParam(8, $challan_date, PDO::PARAM_STR);
		$pre_stmt->bindParam(9, $subtotal, PDO::PARAM_STR);
		$pre_stmt->bindParam(10, $gsttotal, PDO::PARAM_STR);
		$pre_stmt->bindParam(11, $billamount, PDO::PARAM_STR);
		...
		if ($invoice_no != null) {
			for ($i = 0; $i < count($price); $i++) {
				...
				if (isset($total_quantity[$i])) {
					...
					$insert_product = $this->pdo->prepare("INSERT INTO `invoice_details`(`invoice_no`,`pid`, `product_name`,`product_price`, `price`,`gst_price`, `quantity`,`discount`) VALUES (?,?,?,?,?,?,?,?)");
			...
			// $find_customer_stmt = $this->find('member', 'id', $customer_id);
			...
			// $suppliar_update_res = $this->update('member', 'id', $customer_id, $member_update_query);
