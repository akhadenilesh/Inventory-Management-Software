            ...
            <div class="info-box bg-cards-1">
              <div class="info-box-content  text-center text-white">
                ...
                <span class="sell">Sell:
                  <?php
                  $today = date('Y-m-d');
                  $stmt = $pdo->prepare("SELECT SUM(`bill_amount`) FROM `invoice` WHERE `order_date` = '$today'");
                  $stmt->execute();
                  $res = $stmt->fetch(PDO::FETCH_NUM);
                  echo $res[0];
            ...
            <div class="info-box bg-cards-2">
              <div class="info-box-content  text-center">
                ...
                <span class="sell">Sell:
                  ...
                  $end_date =   date('Y-m-t');
                  $stmt = $pdo->prepare("SELECT SUM(`bill_amount`) FROM `invoice` WHERE `order_date` BETWEEN '$start_data' AND  '$end_date' ");
                  $stmt->execute();
                  $res = $stmt->fetch(PDO::FETCH_NUM);
                  echo $res[0];
          ...
          <div class="card">
            ...
            <div class="card-body">
              <div class="responsive">
                <table id="stockAlertTable" class="display dataTable text-center">
                  <thead>
                    <tr>
                      <th>#</th>
                      <th>ID</th>
  ...
  <script>
    $(document).ready(function() {
      var table = $('#stockAlertTable').DataTable({
        "paging": true,
        "searching": true
      });

      // Custom search filter on product name
      $('#searchProductName').on('keyup', function() {
        table.column(2).search(this.value).draw(); // column(2) is "Product Name"
      });
    });
  </script>